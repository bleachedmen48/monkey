...
a.Name = "Nigga"
a.Value = game.Players.LocalPlayer.Team
a.Parent = game.Players.LocalPlayer

Player = game.Players.LocalPlayer
game:GetService("RunService").RenderStepped:Connect(function()
if game:GetService("CoreGui"):FindFirstChild("PurchasePrompt") then
game:GetService("CoreGui"):WaitForChild("PurchasePrompt"):remove()
end end)
...
local Tab1 = Window:CreateTab("Military Tycoon")
...
getgenv().Speed = 300
local Toggle1 = Section1:CreateToggle("AFK Mode", nil, function(State)
toggle = State
    ...
    pcall(
        function()
            for i, v in pairs(game:GetService("Workspace").Tycoons:GetChildren()) do
                if v.ClassName == "Model" and v.Name:match(tostring(Player.Nigga.Value)) then
                    wait(1)

                    local Time =
                        (v.Essentials.Giver.CollectButton.CFrame.p + Vector3.new(0, 0, 0) -
                    ...
                    local Tween =
                        ...
                        Info,
                        {CFrame = CFrame.new(v.Essentials.Giver.CollectButton.CFrame.p + Vector3.new(0, 5, 0))}
                    )
                    Tween:Play()
            ...
            for _, v in pairs(
                game:GetService("Workspace").Tycoons[tostring(game.Players.LocalPlayer.Nigga.Value)].Buttons:GetChildren(

                )
            ) do
                if v:IsA("Model") and v.Button.Color == Color3.fromRGB(0, 255, 0) and v.Button.Transparency == 0 then
                    local Time =
                        (v.Button.CFrame.p + Vector3.new(0, 0, 0) -
                        game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude / getgenv().Speed
                    local Info = TweenInfo.new(Time, Enum.EasingStyle.Linear)
                    local Tween =
                        game:GetService("TweenService"):Create(
                        game.Players.LocalPlayer.Character.HumanoidRootPart,
                        Info,
                        {CFrame = CFrame.new(v.Button.CFrame.p + Vector3.new(0, 5, 0))}
                    )
                    Tween:Play()
                end
            end
...
local Slider3 = Section4:CreateSlider("TweenSpeed",0,500,nil,false, function(Value)
	getgenv().Speed = Value
end)
